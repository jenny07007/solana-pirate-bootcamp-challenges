...
/**
 * Helper function to upload token image to bundlr and get metadata uri
 */
export async function uploadImagAndGetMetadaUri(
  connection: Connection,
  payer: Keypair,
  fileName: string,
  tokenName: string,
) {
  const buffer = fs.readFileSync(`./assets/${fileName}`);
  ...
  const { uri: metadataUri } = await metaplex.nfts().uploadMetadata({
    name: tokenName,
    image: imageUri,
    description: `${tokenName} token for Pirate Ship on the Seven Sea`,
  });
